...
exports.import = function(config) {
  ...
  // function to create visuals from files
  function createVisualFromFile(file, remotePath) {
    var stats = fs.statSync(file);
    var visual = {
    ...
    return nuxeo.repository()
        .create(remotePath, visual)
        .then((doc) => {
          if (config.verbose) {
            console.log('Created visual: ' + doc.path);
          }
          return nuxeo.operation('Blob.AttachOnDocument')
              .param('document', doc.path)
              .input(blob)
              .execute({ schemas: ['dublincore', 'file', 'Picture']})
              .then((doc) => {
                if (config.verbose) {
                ...
                if (config.verbose) {
                  console.log('Task in error for blob attach: ' + file);
                }
                filesNotWellProcessed.push({ path: file, err: error });
              });
        }).catch((error) => {
          if (config.verbose) {
            console.log('Task in error for: ' + dir);
          }
          filesNotWellProcessed.push({ path: dir, err: error });
        });
  }
  ...
  function createDocument(opts) {
    ...
    if (isDirectory) {
      ...
      // just going to accept some file types
      var isVisual = ['.png', '.jpg', '.jpeg', '.bmp', '.gif'].includes(path.extname(opts.absPath));
      filesCount++;
      if (isVisual) {
        if (config.verbose) {
          console.log('Is visual: ' + opts.absPath);
        }
        return createVisualFromFile(opts.absPath, opts.remotePath);
      } else {
        if (config.verbose) {
          console.log('Is file: ' + opts.absPath);
        }
